...
class   AcnooOrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            'coupon' => 'nullabled',
            'type' => 'in:reels|post|story',
            'headline' => 'string',
            'hashtags' => 'string',
            'instagram_mentions' => 'string',
            'content_category' => 'string',
            'discount_amount' => 'nullable',
            'total_amount' => 'required_without:coupon|required',
            'start_date' => 'nullable|string',
            'end_date' => 'nullable|string',
            'start_time' => 'nullable|string',
            'end_time' => 'nullable|string',
            'description' => 'nullable|string',
            'influencer_id' => 'required_without:coupon|exists:users,id',
    ...
    public function amountValculation(Request $request){
       $request->validate([
            'coupon' => 'nullabled',
            'type' => 'in:reels|post|story',
            'influencer_id' => 'required_without:coupon'
       ]);  
      
    if ($request->filled('coupon')) {
        $coupon = OrderCoupon::where('code', $request->coupon)->where('status', 'active')->first();
        ...
        if (!$coupon) {
            ...
            return response()->json([
                ...
                // Dynamically access amount from column (e.g. $user->reels or $user->post)
                $user = User::where('id' , $request->influencer_id )->first();
                $basePrice = $user->$type; // e.g. $user->reels if type = 'reels'

                $service_charge = Option::where('key', 'service-charge')->first()->value ?? [];
                $gst = ($basePrice * $request->gst_percentage) / 100;
                $platformFee = ($basePrice * $service_charge) / 100;
                $processingFee = ($basePrice * $request->processing_fee) / 100;

                $total = $basePrice + $gst + $platformFee + $processingFee;
